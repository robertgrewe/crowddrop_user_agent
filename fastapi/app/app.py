...
# Import the initialization function from your agent.py file
from agent import initialize_openapi_agent
...
class AgentQueryRequest(BaseModel):
    message: str = Field(..., example="List all pending tasks.", description="The natural language query to send to the OpenAPI agent.")
...
class AgentResponse(BaseModel):
    response: str = Field(..., example="Successfully retrieved 5 tasks: Task A, Task B, Task C, Task D, Task E.", description="The final response generated by the OpenAPI agent.")
...
class HealthCheckResponse(BaseModel):
    ...
    agent_initialized: bool = Field(..., example=True, description="Indicates if the OpenAPI agent has been successfully initialized.")
...
app = FastAPI(
    ...
    description="An API to interact with an OpenAPI-powered LangChain agent emulating humanoid robot capable of using the CrowdDrop API.",
    version="1.0.0",
)
...
async def startup_event():
    """
    Initializes the OpenAPI agent by calling the function from agent.py
    ...
    print("FastAPI app starting up: Initializing agent...")
    # Unpack all three returned values from initialize_openapi_agent
    agent, token, persona = await initialize_openapi_agent()
    if agent:
        openapi_agent_instance = agent
        global_access_token = token
        global_teslabot_persona = persona # Store the persona
        print("Agent successfully loaded into FastAPI app with persona.")
    else:
...
@app.post(
    ...
    description="Sends a natural language chat message to the underlying LangChain OpenAPI agent, which can interact with the CrowdDrop API.",
    responses={
...
async def chat(request_body: AgentQueryRequest) -> AgentResponse:
    """
    Processes a user query using the initialized OpenAPI agent.
    Includes the Teslabot persona in the query and returns intermediate steps.
    """
    ...
    # Prepend the global persona to the user's query
    ...
    try:
        response = await openapi_agent_instance.ainvoke(
            {"input": user_query_with_persona}, # Use the query with persona
...
@app.get(
    ...
    description="Checks the health and initialization status of the API server and the OpenAPI agent.",
    tags=["Monitoring"]
)
async def health_check() -> HealthCheckResponse:
    """
    Provides a simple health check to indicate if the API is running
    and if the core OpenAPI agent has been successfully initialized.
    """
